...
public class MarkPermutations {
	...
	public MarkPermutations(byte[] c,ElectionSpecification es, Vector<Vector<Integer>> partitions) {
		...
		questions = new byte[qs.length];
		maxNoAnswers = new byte[qs.length];
		totalNoAnswers=0;
		for (int qno = 0; qno < qs.length; qno++) {
			questions[qno] = (byte)qs[qno].getAnswers().size();
			totalNoAnswers+=questions[qno];
		...
		for (int qno = 1; qno < qs.length; qno++) {
			noAnswersUntilQuestion[qno]=noAnswersUntilQuestion[qno-1]+questions[qno-1];
		...
		for (int i=0;i<partitions.size();i++) {
			...
			for (int j:part) {
				noAnswersInPartition[i]+=questions[j];
	...
	public byte[] project(byte[] p, int partitionId) {
		...
		int retPos=0;
		for (int i:partitions.get(partitionId)) {
			System.arraycopy(p, noAnswersUntilQuestion[i], ret, retPos, questions[i]);
			retPos+=questions[i];
		}
		return ret;
	}
		...
		for (int q:partitions.get(partitionId)) {
			...
			for (int a=0;a<getMaxNoAnswersInQuestion(q);a++) {
				if (p3[p3Pos+a]==-1)
					...
					if (p3[p3Pos+a]<-1 || p3[p3Pos+a]>=questions[q]) {
		...
		for (int q:partitions.get(partitionId)) {
			for (int a=0;a<getMaxNoAnswersInQuestion(q);a++) {
				if (d3[d3Pos+a]==-1)
					...
					if (d3[d3Pos+a]<-1 || d3[d3Pos+a]>=questions[q]) {
