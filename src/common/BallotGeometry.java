...
//System.out.println(currentDiffy);
			if (currentDiffy<-0.1)
				return i+1;			
		}
		return q.size();
	...
	public Rectangle[][] getSerialBulleted() {
		...
		float cellWidth=Math.abs(rect.width())/noColumns;
		float cellHeight=Math.abs(rect.height())/noRows;
		
		float zerox=Math.min(rect.left(),rect.right());
		float zeroy=Math.max(rect.top(),rect.bottom());		
