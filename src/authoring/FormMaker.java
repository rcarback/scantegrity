...
public class FormMaker {
	...
	public FormMaker(ElectionSpecification es,BallotGeometry geom) throws Exception, DocumentException {
		...
		helv = BaseFont.createFont("Courier", "winansi", false);
...
*/        
	...
	public PdfFormField makeButtonOrangeTop(Rectangle possition, String name) {
		possition=new Rectangle(possition.left()-(float)geom.getDonutThicknessTop()*72,
				possition.bottom()-(float)geom.getDonutThicknessTop()*72,
				possition.right()+(float)geom.getDonutThicknessTop()*72,
				possition.top()+(float)geom.getDonutThicknessTop()*72);
	...
	public PdfFormField makeButtonHighlightCandidate(Rectangle possition, String name) {
		...
		localPossition.setRight(localPossition.right()+rightSpam*72);
		float h = localPossition.height();
		float w = localPossition.width();
        PdfFormField pushbutton = PdfFormField.createPushButton(writer);
        cb.moveTo(0, 0);
...
//System.out.println(possition.left()+" "+possition.bottom()+" "+possition.right()+" "+possition.top());		
		
		possition=new Rectangle(
				possition.left()-(float)geom.getDonutThicknessTop()*72,
				possition.bottom()-(float)geom.getDonutThicknessTop()*72,
				possition.right()+(float)geom.getDonutThicknessTop()*72,
				possition.top()+(float)geom.getDonutThicknessTop()*72);
...
*/
	...
	public static void drawWhiteRectangleStatic(PdfContentByte cb,Rectangle r) {
		...
		float wider=0.15f;
		cb.rectangle(r.left()-r.width()*wider,r.bottom()-r.height()*wider,r.width()*(1+2*wider),r.height()*(1+2*wider));
		cb.fillStroke();
		cb.restoreState();
	}
    ...
    public static int getFontSize(Rectangle rect, String zero,BaseFont font) {
    	return getFontSize(Math.abs(rect.width()),Math.abs(rect.height()),zero,font);
    }
    ...
    public static int getFontSize(float w, float h,String zero,BaseFont font) {
    	...
    	size--;
    	int returnedSize=(int)(size*0.75);
    	return returnedSize;
    }
