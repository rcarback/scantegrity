...
public class PrintableBallotMaker extends software.authoring.scantegrity.PrintableBallotMaker {
	...
	static Color dummyInvisibleInkColor=new Color(255,255,0);//100% Yellow
	...
	static float decoyOpacity=0.07f;
...
//System.out.println(allCodes);
	...
	protected void addOval(PdfContentByte cb, Rectangle possition) {
		if (possition.height()>possition.width()) {
		...
		//float widthWhite=possition.height()*0.22f;
		float widthWhite=possition.height()*0.32f;
		cb.saveState();
		{
		...
		cb.roundRectangle(possition.left()-widthWhite/2, possition.bottom()-widthWhite/2, possition.width()+widthWhite, possition.height()+widthWhite, possition.height()/2+widthWhite/2);
		cb.stroke();
		cb.restoreState();
		...
		cb.roundRectangle(possition.left(), possition.bottom(), possition.width(), possition.height(), possition.height()/2);
		cb.stroke();
		cb.restoreState();
		...
		cb.setColorStroke(contourColor);
		cb.roundRectangle(possition.left(), possition.bottom(), possition.width(), possition.height(), possition.height()/2);
		cb.stroke();		
		cb.restoreState();		
	}
...
*/	
	...
	public static void addTextCentered(PdfContentByte cb, Rectangle possition, BaseFont font, int XXXfontSize,Color textColor,String text) {
		...
		cb.setColorStroke(Color.WHITE);
		cb.rectangle(possition.left(), possition.bottom(), possition.width(), possition.height());
		cb.fillStroke();
		cb.restoreState();
		
		float rectLeft=possition.left();
		float rectBottom=possition.bottom();
		...
		//StringBuffer bars=new StringBuffer("");
		//for (int i=0;i<=(int)(possition.width()/barWidth);i++) {
			//bars.append(BarChar);
...
//		System.out.println("barWidth="+barWidth+" allBarsWidth="+Math.abs(font.getWidthPoint(bars.toString(),fontSize)));
		...
		float xBar=rectLeft;
		float yBar=rectBottom+(possition.height()-textHeigthPoints)/2;
		
		//float xoffset=(possition.width()-textWidthPoints)/2;
		int noTotalPixels=Math.round((possition.width()/barWidth));
...
//System.out.println(Math.round((possition.width()/barWidth))+" "+(possition.width()/barWidth));		
		int noPixelsSoFar=0;
		float x=xBar,y=yBar;
		String t=null;
...
*/        
    ...
    public static void main(String[] args) throws Exception {
		...
		String background=dir+"policy ballot2.pdf";
