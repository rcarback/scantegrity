...
public class ScannedBallot implements ScannedBallotInterface {
	...
	public void detectFromPerfectImage(BufferedImage img,double dpi) throws Exception {
		...
		detectMarksFormPerfectImage(img, dpi);
	...
	public void detectMarksFormPerfectImage(BufferedImage img,double dpi) {		
		...
		for (Integer raceNo:markedContests.keySet()) {
			...
			if (noMarkedThisContest<qs[raceNo].getMin()&& noMarkedThisContest>0) {
...
*/
	...
	public void detect(BufferedImage img) throws Exception {
		...
		Cluster ulc=null;
		do { 
			ulc=itc.detectCircular(ul.getX(), ul.getY(), 0.7, black);
//System.out.println(ulc);			
		} while (ulc !=null && !isCircle(img, ulc));
		Cluster lrc=null;
		do {
			lrc=itc.detectCircular(lr.getX(), lr.getY(), 0.7, black);			
		} while(ulc !=null && !isCircle(img, lrc));
		
		if (ulc==null)
			throw new Exception("Cannot detect Upper left alignment mark");

		if (lrc==null)
			throw new Exception("Cannot detect lower right alignment mark");
	...
	private BufferedImage computeTransformation(BufferedImage img, Point2D.Double sul, Point2D.Double slr, double dpi) throws Exception {
		...
		//scale
		ret =transform.filter(ret,null); 
		if (debug) {
			try {
				ImageIO.write(ret,"png",new File("Scaled.png"));
			} catch (IOException e) {
				e.printStackTrace();
			}				
		}
		...
		if (debug) {
			try {
				ImageIO.write(ret,"png",new File("Rotated.png"));
			} catch (IOException e) {
				e.printStackTrace();
			}				
		}
		...
		if (debug) {
			try {
				ImageIO.write(ret,"png",new File("Translated.png"));
			} catch (IOException e) {
				e.printStackTrace();
			}				
		}
...
//		ret=ret.getSubimage(0,0,(int)(bgm.getWidth()*150),(int)(bgm.getHeight()*150));
		...
		double alignbmentErrorAllowed = 0.04;//inches
